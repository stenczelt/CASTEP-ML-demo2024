    ...
    train_args.update(
        {
            ...
            "start_swa": start_swa,
            "seed": random_seed,
            "E0s": "{6:-148.1738815378, 14:-163.4954402249}",  # fixme: debug
        }
    )
    if model_kwargs:
...
def convert_to_lammps(model, out_path, head="default"):
    from e3nn.util import jit
    from mace.calculators import LAMMPS_MACE

    model = model.double().to("cpu")
    print(
        f"Selected head: {head} from command line in the list "
        f"available heads: {model.heads}"
    )
    lammps_model = (
