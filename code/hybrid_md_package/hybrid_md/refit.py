...
def refit(state: HybridMD):
    ...
    refit_function_import = refit_settings.function_name
...
def refit_generic(state: HybridMD):
    ...
    # get the atomic numbers present in the training set
    atomic_numbers = set()
    for frame in frames_train:
        frame: ase.Atoms
        atomic_numbers.update(set(frame.get_atomic_numbers()))
    ...
    if refit_settings.descriptor_str is not None:
        ...
        if refit_settings.preset_soap_param is not None:
            desc_str_soap = refit_settings.preset_soap_param.get_turbo_soap_str(
                atomic_numbers, delta
            )
        else:
            desc_str_soap = SoapParamPreset.fast.get_turbo_soap_str(
                atomic_numbers, delta
            )
