...
class HybridMD:
    ...
    def __init__(self, seed: str, md_iteration: int = None):
        ...
        # files of interest
        self.log_filename = f"{self.seed}.ml-energy-temporary-log"
        self.input_filename = f"{self.seed}.ml-energy-input.yaml"
        self.xyz_filename = f"{self.seed}.ml-energy.xyz"
...
class CarriedState(SeedAwareState):
    ...
    * In order to ensure the pre and post steps are called in the right order,
    ...
    def __init__(self, seed: str):
        super().__init__(seed)
        self.state_filename = f"{self.seed}.ml-energy-state.yaml"
...
class SubStateXYZ(SeedAwareState):
    ...
    def _unpack_arrays(frames, key: str):
        # note: np.concat is needed in numpy 2.0
